...
<div id="wrap">
    ...
    <div id="content">
        ...
        <?php $stmt = $db->prepare('select p.id, p.member_id, p.message, p.crated, m.name, m.picture from poste p, members m where p.id=? and m.id=p.member_id order by id desc');
        if (!$stmt) {
            die($db->error);
        }
